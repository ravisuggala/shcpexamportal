...
if(target.equals("storemarks"))
{
	...
	entry=miscUtil.noNull(miscUtil.getFromSession(request, response, "entry"));
...
else
{
	degree=miscUtil.noNull(request.getParameter("degree"));
	 ...
	 entry=miscUtil.noNull(request.getParameter("entry"));
...
session.putValue("students", students);
	...
	{
		session.putValue("option", "externalstep2");
