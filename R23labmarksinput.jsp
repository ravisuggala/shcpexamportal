...
String attmax=miscUtil.getFromSession(request, response,"attmax");
String actvtsmax=miscUtil.getFromSession(request, response,"actvtsmax");
...
for(int i=0;i<mapids.size();i++)
{
	...
	MidExam attempted=medb.mapidSearchinMid(mapid,midno);
	...
	{
		if(attempted.getValue(midno).equals("")&&midno.equals("remid"))
	...
	else
	{
			if(midno.equals("remid")) 
				...
				dmbox="<input id=dm"+i+"  type=text maxlength=5 size=3 name="+mapid+"-dm"+midno+"></input>";
				 qmbox="<input id=qm"+i+" type=text maxlength=5 size=3 name="+mapid+"-qm"+midno+" ></input>";
				 ambox="<input id=am"+i+" type=text maxlength=5 size=3 name="+mapid+"-am"+midno+" ></input>";
				 assbox="<input id=as"+i+" type=text size=5 maxlength=5 name="+mapid+"-as"+midno+" ></input>";
			}
		  ...
		  <div id="centre">
			    ...
			    <CENTER>
						<TABLE BORDER="1" CELLPADDING="2" CELLSPACING="2" WIDTH="300">
							...
							<input type=hidden id="attmax" value='<%=attmax %>'></input>
							<input type=hidden id="actmax" value='<%=actvtsmax %>'></input>
