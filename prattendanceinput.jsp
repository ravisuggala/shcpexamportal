...
String date=session.getValue("date").toString();

String sem=session.getValue("sem").toString();
title=miscUtil.semDescription(sem, degree)+" "+degree+" Attendance Entry";
//midno=session.getValue("midno").toString();
...
for(int i=0;i<students.size();i++)
{
	...
	AttendanceEntry attempted=medb.findAttendanceEntry(regdno,sem,ay,midno);
	...
	if(attempted!=null)
	{
		attbox="<input id=attd"+i+" type=text size=5 maxlength=5 name="+regdno+"-att value="+attempted.getAttendance()+"></input>";
	    atbox="<input type=hidden size=3 name=h"+regdno+"-att value=true></input>";
			 
	}
	else
	{
		attbox="<input id=attd"+i+" type=text size=5 maxlength=5 name="+regdno+"-att></input>";
			
		 atbox="<input type=hidden size=3 name=h"+regdno+"-att value=false></input>";
		  ...
		  <div id="centre">
			    ...
			    <CENTER>
						<TABLE BORDER="1" CELLPADDING="2" CELLSPACING="2" WIDTH="300">
							...
							<th class=label nowrap><font size=1>Attendence %</font>
							...
							<INPUT TYPE=HIDDEN NAME="option" VALUE = "prattendance">
